...
public class RivendellWarriorEntity extends RivendellElfEntity {
	...
	public ILivingEntityData finalizeSpawn(IServerWorld sw, DifficultyInstance diff, SpawnReason reason, ILivingEntityData spawnData, CompoundNBT dataTag) {
		...
		setItemSlot(EquipmentSlotType.FEET, new ItemStack((IItemProvider) LOTRItems.RIVENDELL_BOOTS.get()));
		setItemSlot(EquipmentSlotType.LEGS, new ItemStack((IItemProvider) LOTRItems.RIVENDELL_LEGGINGS.get()));
		setItemSlot(EquipmentSlotType.CHEST, new ItemStack((IItemProvider) LOTRItems.RIVENDELL_CHESTPLATE.get()));
		if (random.nextInt(10) != 0) {
			setItemSlot(EquipmentSlotType.HEAD, new ItemStack((IItemProvider) LOTRItems.RIVENDELL_HELMET.get()));
		}
